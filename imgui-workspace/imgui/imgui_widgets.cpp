...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, ImGuiCol_FrameBg, ImGuiCol_FrameBg, ImGuiCol_FrameBg, ImGuiCol_FrameBg);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, col, style.FrameRounding);

    if (*v) {
        const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
        const float pad = ImMax(1.0f, IM_TRUNC(check_sz / 4.0f));
